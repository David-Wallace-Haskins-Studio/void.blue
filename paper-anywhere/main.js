window.addEventListener('DOMContentLoaded', function () {
  ...
  var brushPreviewContext
  var context
  var timerId = false // false or the ID of the timer for multitouch gestures
  var drawLock = false // prevents multitouch
  var primaryFinger = null // identifier of first finger for touch events
  var spaceIsDown = false // for spacebar dragging
  var cmdIsDown = false // for cmd undo and redo
  var circleIsDown = false // for circle gestures
  var clickListenerList // initialized towards the end.
  var saveTimerId = false
  var gestureStartCoords
  var undoRedoMode
  var o
  var dynamicUndoRedoStartTime
  var dynamicUndoRedoAutoRepeat

  window.mainDrawing = mainDrawing // for debugging
  ...
  function resizeTouchCircle () {
    /* window.requestAnimationFrame(function () {
            var circleRadius = (window.screen.width < window.screen.height) ?
  ...
  function startHandle (event) {
    ...
    function getPosition (obj) {
      var topValue = 0; var leftValue = 0
      while (obj) {
    ...
    function setSlider (event) {
      ...
      // this return value should be a method call of the current user context.
  ...
  function dynamicUndoRedo (curTime) {
    if (undoRedoMode) {
      window.requestAnimationFrame(dynamicUndoRedo)
    }
    var n
    var timeDelta = curTime - dynamicUndoRedoStartTime
  ...
  clickListenerList = [
    ...
    // fancy palette dismissal to be implemented later
    {
      id: 'card',
